...
IC=VARselect(y=newdata10000, type="none",lag.max=15)
...
plot(1:15,criteria[,1],ylab="Information criteriaeria",xlab="Number of lags",type="o",
     ylim=c(-4,4),col=col_v[1],pch=plot_v[1],lty=line_v[1])

for (i in 2:4){
  lines(1:15,criteria[,i],type="o",col=col_v[i],lty=line_v[i],pch=plot_v[i])
}
